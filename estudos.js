// ====================================================================
// LOOPR - ESTUDOS (VERSÃO ATUALIZADA - SEMANA PERSISTENTE)
// ====================================================================

// Tenta carregar a última semana visualizada ou inicia na semana 1
let currentWeek = parseInt(localStorage.getItem('loopr-last-week')) || 1;
let currentDay = 'seg';
...
function renderDay(week, day) {
    ...
    daySubjects.forEach((subject, globalIndex) => {
        ...
        card.querySelector('.save-edit').onclick = () => {
            ...
            // Encontrar o índice real no array global
            const realIndex = subjects.indexOf(subject);
            subjects[realIndex].name = newName;
            subjects[realIndex].time = newTime;
        ...
        card.querySelector('.add-lesson-btn').onclick = e => { 
            e.stopPropagation(); 
            const input = card.querySelector('.lesson-input'); 
            input.classList.remove('hidden'); 
            input.querySelector('.lesson-text').focus(); 
        };

        card.querySelector('.save-lesson').onclick = () => { 
            const input = card.querySelector('.lesson-text'); 
            if (input.value.trim()) { 
                subject.lessons.push(input.value.trim()); 
                saveToStorage(); 
                renderDay(week, day); 
            } 
        };

        card.querySelector('.cancel-lesson').onclick = () => { 
            const input = card.querySelector('.lesson-input'); 
            input.classList.add('hidden'); 
            input.querySelector('.lesson-text').value = ''; 
        };

        card.querySelectorAll('.delete-lesson').forEach(btn => {
            btn.onclick = e => { 
                e.stopPropagation(); 
                subject.lessons.splice(btn.dataset.index, 1); 
                saveToStorage(); 
                renderDay(week, day); 
            };
        });
...
function syncWeekAndRender() {
    ...
    localStorage.setItem('loopr-last-week', currentWeek); // Salva a escolha
    updateTitle(); 
    renderDay(currentWeek, currentDay); 
    updateNavButtons();
}
...
function updateTitle() {
    ...
    const label = currentWeek <= 10 ? ordinals[currentWeek-1] : `${currentWeek}ª`;
    document.getElementById('pageTitle').textContent = `${label} Semana`;
}
